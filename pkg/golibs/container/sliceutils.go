...
// The order of the existing elements may be changed.
// The function does it in place, so the original slice is changed
func SliceRemoveIdxUnstable[V any](slice []V, idx int) []V {
...
// The order of the rest elements in the slice is not changed.
func SliceRemoveIdxStable[V any](slice []V, idx int) []V {
	copy(slice[idx:], slice[idx+1:])
	slice[len(slice)-1] = *new(V)
	return slice[:len(slice)-1]
}

// SliceInsertIdx inserts the value v at index idx of slice. All the elements starting at idx will be shifted right,
// so the operation is stable and keeps the order of the elements.
func SliceInsertIdx[V any](slice []V, v V, idx int) []V {
	if idx == len(slice) {
		return append(slice, v)
	}
	slice = append(slice, *new(V))
...
func SliceTransform[A any, B any](a []A, f func(a A) B) []B {
	if len(a) == 0 {
		return nil
	}
	res := make([]B, len(a))
