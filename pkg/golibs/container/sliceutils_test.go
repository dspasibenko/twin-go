...
func TestSliceRemoveIdxUnstable(t *testing.T) {
	assert.Panics(t, func() {
		SliceRemoveIdxUnstable([]string{}, 0)
	})
	s := []string{"a", "b", "c"}
	assert.Equal(t, []string{"c", "b"}, SliceRemoveIdxUnstable(s, 0))
	s = []string{"a", "b", "c"}
	assert.Equal(t, []string{"a", "c"}, SliceRemoveIdxUnstable(s, 1))
	s = []string{"a", "b", "c"}
	assert.Equal(t, []string{"a", "b"}, SliceRemoveIdxUnstable(s, 2))
}

func TestSliceRemoveIdxStable(t *testing.T) {
	assert.Panics(t, func() {
		SliceRemoveIdxUnstable([]string{}, 0)
	})
	s := []string{"a", "b", "c"}
	assert.Equal(t, []string{"b", "c"}, SliceRemoveIdxStable(s, 0))
	s = []string{"a", "b", "c"}
	assert.Equal(t, []string{"a", "c"}, SliceRemoveIdxStable(s, 1))
	s = []string{"a", "b", "c"}
	assert.Equal(t, []string{"a", "b"}, SliceRemoveIdxStable(s, 2))
}

func TestSliceInsertIdx(t *testing.T) {
	s := []string{"b", "c"}
	assert.Equal(t, []string{"a", "b", "c"}, SliceInsertIdx(s, "a", 0))
	s = []string{"a", "c"}
	assert.Equal(t, []string{"a", "b", "c"}, SliceInsertIdx(s, "b", 1))
	s = []string{"a", "b"}
	assert.Equal(t, []string{"a", "b", "c"}, SliceInsertIdx(s, "c", 2))
}
...
func TestSliceTransform(t *testing.T) {
	assert.Nil(t, SliceTransform([]int{}, func(i int) float32 { return 0.0 }))
	assert.Equal(t, []int{1, 2, 3}, SliceTransform([]float32{1, 2, 3}, func(f float32) int { return int(f) }))
}
